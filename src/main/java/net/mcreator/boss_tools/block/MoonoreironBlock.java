...
public class MoonoreironBlock extends BossToolsModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("moonoreiron", "moonoreiron", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == MoonStoneBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 11)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(5, 1, 1, 64))));
		}
	}
}
