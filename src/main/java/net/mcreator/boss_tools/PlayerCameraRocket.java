	...
	public void setupFOVPlayer(EntityViewRenderEvent.FOVModifier event) {
		...
		if (Minecraft.getInstance().player.getPersistentData().contains("rocketsit")) {
			...
			if (rocketsit) {
				...
				// if
				// (Minecraft.getInstance().gameSettings.getPointOfView().equals(PointOfView.FIRST_PERSON))
				// {
				...
				// if
				// (Minecraft.getInstance().gameSettings.getPointOfView().equals(PointOfView.THIRD_PERSON_FRONT))
				// {
				// event.setFOV(100); // event.setFOV(event.getFOV() - 35);
				// }
				// if
				// (Minecraft.getInstance().gameSettings.getPointOfView().equals(PointOfView.THIRD_PERSON_BACK))
				// {
				// event.setFOV(100); // event.setFOV(event.getFOV() - 35);
				// }
			} else {
